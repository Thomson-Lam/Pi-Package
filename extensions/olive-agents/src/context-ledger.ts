...
export const CONTEXT_RETURN_ENTRY = "olive-agent-context-return";
export const CONTEXT_RETURN_ACK_ENTRY = "olive-agent-context-return-ack";
...
/** A durable, incremental child-to-parent context checkpoint. */
export interface ContextReturnCheckpoint {
  version: 1;
  id: string;
  agentId: string;
  sourceSessionFile?: string;
  sourceSessionName: string;
  createdAt: string;
  reason: "completed" | "turn_limit" | "interrupted" | "manual";
  summary?: string;
  selections: LedgerSnapshot[];
...
/** Child-to-parent checkpoints persisted in a child session. */
export function getContextReturnCheckpoints(entries: SessionEntry[]): ContextReturnCheckpoint[] {
  return readCustom<ContextReturnCheckpoint>(entries, CONTEXT_RETURN_ENTRY, (d) => {
    const raw = d as ContextReturnCheckpoint | undefined;
    return raw?.version === 1 && typeof raw.id === "string" && Array.isArray(raw.coveredEntryIds) ? raw : undefined;
  });
}

export function getContextReturnAcknowledgements(entries: SessionEntry[]): ContextReturnAcknowledgement[] {
  return readCustom<ContextReturnAcknowledgement>(entries, CONTEXT_RETURN_ACK_ENTRY, (d) => {
    const raw = d as ContextReturnAcknowledgement | undefined;
    return raw?.version === 1 && typeof raw.checkpointId === "string" ? raw : undefined;
  });
}

/**
 * Return the child branch eligible for the next outbound checkpoint. The
 * launch user message is excluded because it may embed the inherited parent
 * ledger; checkpoint-covered messages are excluded for incremental delivery.
 */
export function returnableContextEntries(entries: SessionEntry[]): SessionEntry[] {
  const covered = new Set(getContextReturnCheckpoints(entries).flatMap((checkpoint) => checkpoint.coveredEntryIds));
  const launchMessageId = entries.find((entry) => entry.type === "message" && entry.message.role === "user")?.id;
  return entries.filter((entry) =>
    entry.type !== "message" || (entry.id !== launchMessageId && !covered.has(entry.id))
  );
}
...
export function nodeToMarkdown(node: Pick<ContextLedgerNode, "summary" | "selections">): string {
  const parts: string[] = [];
...
export interface FinalizeContextReturnInput {
  agentId: string;
...
export function finalizeContextReturn(input: FinalizeContextReturnInput): ContextReturnCheckpoint {
  const messageIds = input.branch.filter((entry) => entry.type === "message").map((entry) => entry.id);
  return {
    version: 1,
    id: input.checkpointId ?? newLedgerNodeId(),
