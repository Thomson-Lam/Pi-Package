...
- `src/memory.ts`, `src/model-resolver.ts`, `src/enabled-models.ts`, `src/prompts.ts`, `src/usage.ts` — supporting runtime behavior.
...
- `test/` — coverage for manager lifecycle/queue/restore, context ledger, `/ot`, approval, mailbox, model resolution, memory, tmux, and tool registration.
