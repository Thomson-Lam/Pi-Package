...
- `src/index.ts` — registers `/agents`, `/agent-session`, `/mag`, the `alt+a` shortcut, and the `Agent` / `get_subagent_result` tools; wires managers, approval, and UI.
...
- `src/ui/` — fleet overview (`fleet-list.ts`) and formatting (`format.ts`).
...
- `test/` — unit coverage for manager, runner, approval, mailbox, model resolution, and UI.
