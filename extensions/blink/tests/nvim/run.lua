...
state.add(model, { versionId = 3, fileId = "f", displayPath = "a.txt", snapshotPath = "/tmp/3", originKind = "file", originSnapshotPath = "/tmp/o" }, true)
eq(model.activeVersionId, 1, "pin prevents follow")
eq(model.by_id[3].unread, true, "pinned update unread")
state.evict(model, 1)
eq(model.activeVersionId, 2, "eviction selects nearest")
eq(vim.tbl_map(function(v) return v.versionId end, model.versions), { 2, 3 })
eq(state.navigate_file(model, 2, -1).versionId, 3, "file navigation wraps")
