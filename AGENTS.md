...
- `extensions/ctx/src/index.ts` — registers `/ctx`, `/cnew`, `/cb`, and `/reads`; `src/fresh/` owns the prepared fresh-session workflow (`muon-build.ts` glues `/cb` to Muon build mode via a dynamic-imported `muon-state` entry), `src/collector.ts`, `src/reconstruct.ts`, and `src/state.ts` collect and rebuild context metadata, while `src/ui/` owns panels and widgets.
