...
- `extensions/olive-agents/index.ts` — forwards to `src/index.ts`, which registers `/agents`, `/agent-session`, `/mag`, the `alt+a` shortcut, and the `Agent` / `get_subagent_result` / `steer_subagent` tools.
