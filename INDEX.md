...
- `extensions/olive-agents/index.ts` — forwards to `src/index.ts`, which registers `/agents`, `/agent-session`, `/ot`, `/otn`, the `alt+a` shortcut, and the `Agent` / `get_subagent_result` tools; `/ot` can start agents from selected ledger contexts and `/otn` starts an unlimited agent from current-session context; persists the context ledger (selected messages/tools + non-mutating compaction output) for each launch and rebuilds the tree via `/ot` after resume.
