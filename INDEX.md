...
- `extensions/olive-agents/index.ts` — forwards to `src/index.ts`, which registers `/agents`, `/agent-session`, `/ot`, `/otn`, `/ocl`, the `alt+a` shortcut, and the supervised detached `Agent` tool; `/ot` can start or reopen agents from selected ledger contexts and `/otn` starts an agent from current-session context; child-local decisions return context only when the human chooses Return.
